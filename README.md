...
- 飞书 CLI：npm 包 `@larksuite/cli`；脚本会额外检测 `lark-cli auth status`，区分用户身份和 Bot 身份。如果检测到 Hermes 上下文但还没绑定，会先提示运行 `lark-cli config bind --source hermes --identity <bot-only|user-default>`；如果用户身份未登录，会继续提示运行 `lark-cli auth login --recommend` 完成推荐权限授权
